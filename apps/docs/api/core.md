...
موتوری که پشت فیلدهای قابل‌تایپِ date pickerها کار می‌کند. `applyFormatMask` رقم‌های
تایپ‌شده را داخل یک الگوی format می‌ریزد تا کاربر هیچ جداکننده‌ای تایپ نکند، و آن‌ها
را با ارقام همان locale نمایش می‌دهد. pickerها خودشان آن را صدا می‌زنند؛ فقط وقتی
مستقیم سراغش بروید که فیلد اختصاصی خودتان را می‌سازید.
...
سه قاعده باعث می‌شود رفتارش مثل یک فیلد تاریخ نیتیو باشد:

- **فیلد پر که شد، نوبت بعدی است.** به‌محض اینکه `MM` دو رقم گرفت، رقم بعدی `DD` را
  شروع می‌کند.
- **فیلدی که رقم بعدی در آن جا نمی‌شود هم رد می‌شود.** `95` ماه نیست، پس `9` می‌شود
  ماه `09` و `5` روز را شروع می‌کند.
- **جداکنندهٔ تایپ‌شده فیلد را زودتر می‌بندد.** `1402-1-2` یعنی ماه ۱ و روز ۲، نه ماه
  ۱۲ — و هر جداکننده‌ای که تایپ کنید به جداکنندهٔ خودِ format تبدیل می‌شود.

| آرگومان            | نوع                | توضیح                                                        |
| ------------------ | ------------------ | ------------------------------------------------------------ |
| `value`            | `string`           | متن فعلی، با ارقام لاتین یا فارسی/عربی                       |
| `format`           | `string`           | الگوی format، با همان tokenهای `DoranDate.format`            |
| `options.locale`   | `Locale \| string` | ارقامی که با آن رندر می‌شود؛ پیش‌فرض locale جهانی            |
| `options.caret`    | `number`           | موقعیت caret داخل `value`؛ پیش‌فرض انتهای متن                |
| `options.previous` | `string`           | مقدار پیش از این ویرایش — تا backspace از جداکننده‌ها رد شود |

خروجی `{ text, caret }` است: متنی که باید نمایش داده شود و جایی که caret باید برود.

متنی که رقم و جداکننده نیست — یک جمله، یا نام ماهی که در حال تایپ است — دست‌نخورده
برمی‌گردد تا فیلد بتواند آن را نامعتبر علامت بزند نه اینکه نابودش کند. formatهایی که
token متنی دارند (`MMMM`، `dddd`، `A`، `Z`) اصلاً قابل ماسک‌شدن نیستند؛
`isMaskableFormat(format)` همین را گزارش می‌کند و آن فیلدها آزاد می‌مانند و روی blur
مرتب می‌شوند.
