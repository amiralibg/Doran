...
نسبی) و **سبک ارقام** (فارسی `۱۴۰۵` در برابر لاتین `1405`). دوران این دو را جدا نگه
می‌دارد تا یک UI انگلیسیِ LTR بتواند نام ماه فارسی را با ارقام لاتین — یا هر ترکیب
دیگری — نشان دهد، بدون جنگیدن با سیستم locale.
...
> یک **provider** برای React/SSR که locale را به یک زیردرخت محدود کند (به‌جای سراسری)
> جداگانه دنبال می‌شود. تا آن زمان، `setDefaultLocale` کلید سطح‌اپ است و
> prop/option به نام `locale` همان override در محل استفاده.
...
عوض‌کردن به `enUS` برای گرفتن ارقام لاتین، نام ماه/روز را هم به انگلیسی عوض می‌کند.
وقتی نام فارسی با ارقام لاتین می‌خواهید (رایج در UIهای دوزبانه)، کلید `digits` در هر
فراخوانی مسیر ارگونومیک است — بدون نیاز به locale سفارشی.
