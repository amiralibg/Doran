...
Idiomatic **Svelte** bindings for [Doran](https://github.com/amiralibg/Doran) — the Persian (Jalali) calendar. Works with Svelte 4 and 5, including SvelteKit SSR.

The components are thin `bind:value` wrappers over the [`@doranjs/wc`](https://github.com/amiralibg/Doran/tree/main/packages/wc) custom elements (the shared engine), so the calendar/grid logic isn't reimplemented per framework. The change convention matches [`@doranjs/react`](https://github.com/amiralibg/Doran/tree/main/packages/react): `bind:value` carries a `DoranDate`, and `change` **also** reports the Gregorian `Date`.
...
Any attribute the underlying element supports (`locale`, `placeholder`, `format`, `with-time`, `min`, `max`, …) passes through via `$$restProps`.

## Headless — `createCalendarGrid`

For fully custom markup, the store reuses the shared `buildMonthGrid` / `navigateFocus` from `@doranjs/wc` — no per-framework grid logic:
...
> Part of the framework-bindings effort ([#22](https://github.com/amiralibg/Doran/issues/22)), alongside [`@doranjs/vue`](https://github.com/amiralibg/Doran/tree/main/packages/vue). Angular bindings follow.
